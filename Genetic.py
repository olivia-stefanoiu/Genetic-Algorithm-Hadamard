...
Target_y = 0.5
populationSize = 6
populationConst = populationSize
generation = 2
chromosome_size = 4
...
class Chromosome:
    def __init__(self, initial_genes=None):
        self.genes = initial_genes if initial_genes is not None else np.array(
            [secrets.choice(Genes) for _ in range(INITIAL_CROMO_SIZE)])
...
def crossover_candidates(population):
    ...
    for i in range(len(relative_fitnesses)):
        probability.extend([i] * int(relative_fitnesses[i] * 100000.0))  # !!! does not reach 100 sadly
    for i in range(populationSize):
        secret = secrets.choice(probability)
        chromosomes = population[secret]
...
def calculate_population_ct(): #calculate the amount to multiply to get
    global populationConst
...
def main():
    ...
    for _ in range(NUMBER_GENERATIONS):
        ...
        genes_on_line= np.sqrt(chromosome_size)
